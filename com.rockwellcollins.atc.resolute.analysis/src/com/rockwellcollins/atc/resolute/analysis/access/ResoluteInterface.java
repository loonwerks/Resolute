...
//			}
	...
	public List<EObject> getDefinitions(EObject resoluteLibrary) {
		if (resoluteLibrary instanceof ResoluteLibrary) {
			final List<EObject> definitions = new ArrayList<>();
			((ResoluteLibrary) resoluteLibrary).getDefinitions().forEach(d -> definitions.add(d));
			return definitions;
		}
		return null;
	}
	...
	public List<NamedElement> getArgs(EObject functionDefinition) {
		if (functionDefinition instanceof FunctionDefinition) {
			final List<NamedElement> args = new ArrayList<>();
			((FunctionDefinition) functionDefinition).getArgs().forEach(a -> args.add(a));
			return args;
		}
		return null;
	}
